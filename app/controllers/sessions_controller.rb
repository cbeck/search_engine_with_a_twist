...
class SessionsController < ApplicationController
  ...
  ssl_allowed :create, :new
  before_filter :one_column, :hide_bookmarks
  
  # open_id_consumer :required => [:email, :nickname]
  # open_id_consumer :required => [:email]
  ...
  def create
    ...
    if logged_in?
      ...
      remember_ip_address(request.remote_ip)
  ...
  #       redirect_to open_id_response.redirect_url((request.protocol + request.host_with_port + "/"), complete_sessions_url)
  #     else
  ...
  #       flash[:notice] = "Please add more information to your identity at #{open_id_response.identity_url}.  You need a nickname and email address."
  # 
