...
describe Admin::TopicsController do
  ...
  describe "handling GET /topics/1/edit" do
    ...
    before(:each) do
      @topic = mock_model(Topic)
      @root_topic = mock_model(Topic)
      Topic.stub!(:roots).and_return([@root_topic])
      Topic.stub!(:find).and_return(@topic)
      Topic.stub!(:order_roots).and_return([@root_topic])
    end
  ...
  describe "handling POST /topics" do
    ...
    before(:each) do
      @topic = mock_model(Topic, :to_param => "1", :name => "foobar")
      Topic.stub!(:new).and_return(@topic)
    end
    ...
    describe "with successful save" do
      ...
      def do_post
        ...
        @topic.should_receive(:update_attribute)
        post :create, :topic => {}
      end
      ...
      it "should redirect to the new topic" do
        do_post
        response.should redirect_to(admin_topic_url("1"))
      end
    ...
    describe "with failed save" do
      ...
      def do_post
        ...
        @topic.should_receive(:update_attribute).with(:root_id,@topic.id)
        post :create, :topic => {}
      end
